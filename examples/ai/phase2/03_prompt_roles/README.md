...
- 模型：`gpt-5.4-mini`
- API Key：源码只保留 `replace-with-your-api-key` 占位符；真实 Key 仅在本地临时配置，不提交到 Git

默认占位符运行时只提示 `API Key 未配置`，不会连接本地网关或产生模型请求。
...
1. TODO 1：定义两个受众不同的 System Prompt，并保持其他变量一致。
2. TODO 2：校验占位 Key，并创建关闭 Store 和自动重试的模型。
3. TODO 3：为每个场景按 System → User 组装独立消息。
4. TODO 4：使用相同 Generate 流程依次调用两个场景。
5. TODO 5：提取非空 AssistantGenText。
6. TODO 6：输出并比较术语、复杂度和受众差异。

## 开始练习

```bash
go run ./examples/ai/phase2/03_prompt_roles
```
...
- 两个场景只改变 System Prompt，User Prompt、模型和调用方式完全一致。
- 默认占位 Key 安全退出；真实调用错误保留底层原因。
- 能从完整 AgenticMessage 中提取非空文本，并清楚解释两组回答差异。
...
```bash
gofmt -w examples/ai/phase2/03_prompt_roles/main.go
go test -timeout=60s ./...
go vet ./...
```
