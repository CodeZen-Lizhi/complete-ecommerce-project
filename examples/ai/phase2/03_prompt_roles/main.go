...
// main 使用相同 User Prompt 对比两个 System Prompt 的回答差异。
func main() {
	ctx, cancel := context.WithTimeout(context.Background(), requestTimeout)
	defer cancel()

	// TODO 1：定义两个受众不同的 System Prompt，并保持 User Prompt、模型和调用方式一致。
	...
	// TODO 2：创建关闭服务端 Store 和自动重试的 AgenticModel，并检查占位 API Key。
	startedAt := time.Now()
	...
	for _, scenario := range scenarios {
		...
		// TODO 5：从完整 AgenticMessage 中提取非空 AssistantGenText。
		...
		// TODO 6：输出场景、System Prompt 和回答，完成后对比术语、复杂度和受众差异。
		fmt.Printf("场景：%s\n", scenario.name)
...
// newAgenticModel 校验配置，并创建关闭服务端 Store 和自动重试的 ResponsesModel。
...
// generate 校验输入并执行一次阻塞式 AgenticModel 调用。
func generate(
	ctx context.Context,
...
// assistantText 提取并拼接响应中的 AssistantGenText 内容块。
